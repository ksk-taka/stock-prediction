...
環境変数の一覧は [docs/COMMANDS.md](docs/COMMANDS.md) を参照。

## コマンド

全コマンドの一覧は **[docs/COMMANDS.md](docs/COMMANDS.md)** を参照。

```bash
npm run dev              # 開発サーバー起動
npm run build            # 本番ビルド
npm run monitor:signals  # シグナル検出 → 分析 → Slack通知
npm run slack:bot        # Slackボット常駐起動
```
