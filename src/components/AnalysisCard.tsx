...
export default function AnalysisCard({ analysis, loading }: AnalysisCardProps) {
  ...
  if (loading) {
    return (
      <div className="rounded-lg bg-white p-4 shadow">
        <h3 className="mb-4 text-lg font-semibold text-gray-900">AI分析</h3>
  ...
  return (
    <div className="rounded-lg bg-white p-4 shadow">
      ...
      <p className="mb-4 text-sm text-gray-700">{analysis.summary}</p>
      ...
      <ul className="space-y-1">
        ...
        {(tab === "opportunities"
          ...
          <li className="text-sm text-gray-400">データなし</li>
        )}
      </ul>

      {analysis.priceTarget && (
        <div className="mt-4 flex gap-4 rounded bg-gray-50 p-3">
          <div>
            <span className="text-xs text-gray-500">短期目標</span>
            <p className="text-sm font-semibold text-gray-900">
              {analysis.priceTarget.short.toLocaleString()}
            </p>
          </div>
          <div>
            <span className="text-xs text-gray-500">中期目標</span>
            <p className="text-sm font-semibold text-gray-900">
