/**
 * Gemini 分析用プロンプト生成モジュール
 *
 * Yahoo Finance からデータを取得し、Gemini に送信するための
 * Markdown プロンプトを生成する。
 *
 * CLIスクリプト (generate-gemini-prompt.ts) と
 * API ルート (analyze-full/route.ts) の両方から使える。
 */
...
/**
 * 銘柄の分析プロンプトを生成する
 * @returns プロンプトテキスト (Markdown)
 */
export async function generateAnalysisPrompt(symbol: string): Promise<string> {
  // 期間計算
  ...
  // データ取得（並列）
  const [quote, summary, chart, chart3y, bsResult, dividendHistory] =
    await Promise.all([
      yf.quote(symbol),
  ...
  const closes6m = allCloses
    .filter((r) => r.date >= cutoff6m)
    .map((r) => r.close);
  const closes1y = allCloses
    .filter((r) => r.date >= cutoff1y)
    .map((r) => r.close);
  ...
  {
    const divRows =
      (dividendHistory as unknown as { date: Date; dividends: number }[]) ??
      [];
    const validDivs = Array.isArray(divRows)
      ? divRows.filter((r) => r.dividends > 0)
      : [];
    ...
    if (validDivs.length >= 2) {
      ...
      if (years.length >= 2) {
        ...
        const latestGrowthPct =
          years[1][1] > 0
            ? Math.round(
                ((years[0][1] - years[1][1]) / years[1][1]) * 1000,
              ) / 10
            : null;

        const yearDetails = years
          .slice(0, 5)
          .map(([y, amt]) => `${y}年: ${fmt(amt)}円`)
          .join(", ");
  ...
  if (ohlcv.length >= 2) {
    ...
    const trendDir =
      trendPct > 3 ? "上昇" : trendPct < -3 ? "下落" : "横ばい";
    observations.push(
  ...
  if (w52High > 0) {
    ...
    if (Math.abs(distFromHigh) < 2) {
      observations.push(
        `**52週高値圏**: 現在値 ${fmt(price, 0)}円 は52週高値 ${fmt(w52High, 0)}円 に近い (${distFromHigh.toFixed(1)}%)`,
      );
    }
  }
